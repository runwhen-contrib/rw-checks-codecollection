...
def serve(
    ...
    while max_iterations is None or iterations < max_iterations:
        iterations += 1
        _poll_once(session, relay, pool_id, workdir, capability, log)


def _poll_once(session, relay: str, pool_id: str, workdir: Path, capability, log) -> None:
    try:
        resp = session.post(
            f"{relay}/v1/tasks/next", json={"poolId": pool_id}, timeout=POLL_TIMEOUT
        )
